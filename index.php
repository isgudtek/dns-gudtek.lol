...
// Fetch last 12 registered domains (3 rows × 4 columns)
$db = new SQLite3(__DIR__ . '/data.db');
$lastDomainsQuery = $db->query('SELECT slug FROM redirects WHERE active = 1 ORDER BY id DESC LIMIT 12');
$lastDomains = [];
while ($row = $lastDomainsQuery->fetchArray(SQLITE3_ASSOC)) {
    $lastDomains[] = $row['slug'];
}
$db->close();

// Fallback if less than 12 domains
while (count($lastDomains) < 12) {
    $lastDomains[] = null;
}
?>
<!DOCTYPE html>
    ...
    <div class="container">
        ...
        <div class="hero">
            ...
            <div style="display: flex; gap: 0.75rem; flex-wrap: wrap; justify-content: center; margin-top: 1rem;">
                <button onclick="showHowItWorksModal()" style="max-width: 300px;">HOW DOES IT WORK?</button>
                <button onclick="window.showRoadmapModal()" style="max-width: 300px;">⚡ ROADMAP</button>
            </div>
        ...
        <div class="sponsors">
            <h3>&gt; Last Domains</h3>
            <?php
            // Split into 3 rows of 4 domains each
            $rows = array_chunk($lastDomains, 4);
            foreach ($rows as $row):
            ?>
                <div class="sponsor-grid">
                    <?php foreach ($row as $domain): ?>
                        <?php if ($domain): ?>
                            <a href="https://<?= htmlspecialchars($domain) ?>.gudtek.lol" target="_blank" class="sponsor-box"><?= htmlspecialchars($domain) ?></a>
                        <?php else: ?>
                            <div class="sponsor-box" style="opacity: 0.3;">-</div>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </div>
            <?php endforeach; ?>
        ...
        <div id="contactSection" class="section" style="margin-top: 3rem; scroll-margin-top: 2rem;">
        ...
        <div style="margin-top: 2rem; padding: 0 0 1.5rem 0; text-align: center; opacity: 0.8; font-size: 0.9rem; border-top: 1px solid rgba(0, 255, 0, 0.2);">
            <div>
                <a href="https://gudtek.lol" target="_blank" rel="noopener noreferrer">
                    <img src="https://avatars.githubusercontent.com/u/252833529" alt="Avatar" style="width: 135px; height: auto;">
                </a>
            </div>
            <p style="display: flex; align-items: center; justify-content: center; gap: 1rem; flex-wrap: wrap;">
                ...
                <a href="/privacy.php" style="color: var(--crt-green); text-decoration: none; transition: all 0.3s ease;" onmouseover="this.style.textShadow='0 0 15px var(--crt-glow)'" onmouseout="this.style.textShadow=''">Privacy & Legal</a>
                <span style="color: rgba(0, 255, 0, 0.3);">•</span>
